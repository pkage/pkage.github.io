...
const windowMovement = (win, title) => {
...
const windowResizing = (win, handle) => {
...
const redrawTaskbarMain = () => {
    ...
    const createBtn = winfo => {
        ...
        btn.addEventListener('click', () => {
            windowFocus(document.querySelector(`.window[data-name="${winfo.name}"]`))
        })
        return btn
    }
...
const openWindow = ({name, icon, title, resizable, x, y}, body, cb) => {
    ...
    windowMovement(win, win.querySelector('.title-bar'))
    windowResizing(win, win.querySelector('.window-resize-handle'))
    windowOrdering(win)
    windowButtons( win)
